    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-3 border-b border-slate-100 flex gap-3">
            <form method="GET" action="{{ route('app.varian-produk.index') }}" class="flex flex-1 gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="search" name="q" value="{{ request('q') }}" placeholder="Cari nama varian atau produk..."
                ...
                @if (request()->anyFilled(['q', 'produk_id']))
                    <a href="{{ route('app.varian-produk.index') }}" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 transition whitespace-nowrap">Reset</a>
                @endif
            </form>
        </div>
            ...
            <table class="w-full">
                <thead class="bg-slate-50">
                    <tr class="text-left text-xs uppercase tracking-wide text-slate-500">
                        ...
                        <th class="px-6 py-3 font-medium">Nama Varian</th>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center text-sm text-slate-400">
                                Belum ada varian.
                                <a href="{{ route('app.varian-produk.create') }}" class="text-emerald-600 hover:underline">Tambahkan sekarang.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if ($variants->hasPages())
            <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                <p class="text-xs text-slate-400">Menampilkan {{ $variants->firstItem() }}–{{ $variants->lastItem() }} dari {{ $variants->total() }} varian</p>
                {{ $variants->links() }}
            </div>
        @endif
    </div>
