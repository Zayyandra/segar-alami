        ...
        <div class="px-6 py-3 border-b border-slate-100 flex gap-3">
            <form method="GET" action="{{ route('app.bahan-keluar.index') }}" class="flex flex-1 gap-3 flex-wrap">
                ...
                @if (request()->anyFilled(['bahan_baku_id', 'tanggal']))
                    <a href="{{ route('app.bahan-keluar.index') }}" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 transition">Reset</a>
                @endif
            </form>
        </div>
                    ...
                    @forelse ($bahanKeluars as $item)
                        <tr class="hover:bg-slate-50/50">
                            ...
                            <td class="px-6 py-4 text-right">
                                <span class="text-sm font-bold text-red-500 tabular-nums">−{{ number_format($item->jumlah, 0, ',', '.') }}</span>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center text-sm text-slate-400">
                                ...
                                <a href="{{ route('app.bahan-keluar.create') }}" class="text-emerald-600 hover:underline">Catat sekarang.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
