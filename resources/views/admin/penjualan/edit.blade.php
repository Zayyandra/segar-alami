<x-layouts.admin title="Edit Transaksi">
    <div class="max-w-4xl mx-auto space-y-5">
        ...
        <div>
            ...
            <h1 class="mt-2 text-2xl font-semibold text-slate-900">Edit Transaksi</h1>
            <p class="mt-1 text-sm text-slate-500">{{ $penjualan->tanggal->format('d M Y') }}</p>
        </div>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="keterangan"
                               value="{{ old('keterangan', $penjualan->keterangan) }}"
                                        ...
                                        <select name="items[{{ $i }}][varian_produk_id]"
                                            ...
                                            @foreach ($variants as $v)
                                                <option value="{{ $v->id }}" data-harga="{{ $v->harga }}"
                                                    ...
                                                    {{ $v->produk->nama }} — {{ $v->nama_varian }}{{ $v->ukuran ? ' ('.$v->ukuran.')' : '' }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </td>
