<x-layouts.admin title="Produk" subtitle="Kelola produk Segar Alami.">
    ...
    <div class="mb-6">
        <div class="bg-slate-50 border border-slate-200 rounded-2xl px-6 py-5 flex items-center justify-between">
            ...
            <span class="text-xs font-semibold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full border border-emerald-200">
                Aktif
            </span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-3 border-b border-slate-100 flex gap-3">
            <form method="GET" class="flex flex-1 gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="search" name="search" value="{{ request('search') }}" placeholder="Cari nama produk..."
                ...
                @if (request()->anyFilled(['search', 'kategori']))
                    <a href="{{ route('app.produk.index') }}" class="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-100 transition">Reset</a>
                @endif
            </form>
        </div>
                        ...
                        <tr class="hover:bg-slate-50/50">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                        <div class="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center shrink-0 text-slate-400 text-xs">—</div>
                                    @endif
                                    <div>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex items-center gap-3">
                                    <a href="{{ route('app.produk.edit', $item) }}" class="text-sm font-medium text-emerald-600 hover:text-emerald-700">Edit</a>
                                    <form method="POST" action="{{ route('app.produk.destroy', $item) }}" class="inline"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-16 text-center text-sm text-slate-400">
                                Belum ada produk. <a href="{{ route('app.produk.create') }}" class="text-emerald-600 hover:underline">Tambahkan sekarang.</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if ($produk->hasPages())
            <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                <p class="text-xs text-slate-400">Menampilkan {{ $produk->firstItem() }}–{{ $produk->lastItem() }} dari {{ $produk->total() }} produk</p>
                {{ $produk->links() }}
            </div>
        @endif
    </div>
