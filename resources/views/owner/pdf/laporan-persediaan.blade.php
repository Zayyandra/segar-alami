...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-table { width: 100%; border-collapse: separate; border-spacing: 8px; margin: 0 0 12px 0; padding: 0 22px; }
        .stat-card { padding: 12px 14px; border-radius: 6px; border: 1px solid #e2e8f0; vertical-align: top; width: 25%; }
        .stat-card-red { padding: 12px 14px; border-radius: 6px; border: 1px solid #fecaca; background: #fee2e2; vertical-align: top; width: 25%; }
        .stat-label { font-size: 9px; text-transform: uppercase; letter-spacing: 0.05em; color: #64748b; margin-bottom: 4px; }
        .stat-label-red { font-size: 9px; text-transform: uppercase; letter-spacing: 0.05em; color: #991b1b; margin-bottom: 4px; }
        .stat-value { font-size: 18px; font-weight: bold; color: #1e293b; }
        .stat-value-green { font-size: 18px; font-weight: bold; color: #10b981; }
        .stat-value-red { font-size: 18px; font-weight: bold; color: #dc2626; }
        .stat-value-sm { font-size: 13px; font-weight: bold; color: #1e293b; }
        .stat-sub { font-size: 9px; color: #94a3b8; margin-top: 3px; }
        .stat-sub-red { font-size: 9px; color: #f87171; margin-top: 3px; }
        ...
        table.data-table th { padding: 8px 10px; text-align: left; font-size: 9px; text-transform: uppercase; letter-spacing: 0.05em; color: #64748b; font-weight: 600; }
        table.data-table td { padding: 8px 10px; font-size: 10px; color: #374151; border-bottom: 1px solid #f1f5f9; }
        ...
        .badge-unknown { background: #f1f5f9; color: #64748b; padding: 2px 8px; border-radius: 20px; font-size: 9px; }

        .footer { margin: 20px 30px 0; padding-top: 12px; border-top: 1px solid #e2e8f0; }
...
<body>
            ...
            <tbody>
                @foreach ($bahanBakus as $i => $item)
                    ...
                    <tr>
                        ...
                        <td>{{ $item->satuan }}</td>
                        <td class="right {{ $status === 'kritis' ? 'kritis' : '' }}">
                            {{ number_format($item->stok_saat_ini, 0, ',', '.') }}
                        </td>
                        <td class="right">{{ number_format($item->stok_minimum, 0, ',', '.') }}</td>
                        <td>
                            @if ($status === 'aman')
    ...
    <div class="footer">
        <table class="footer-table">
            <tr>
                <td class="footer-left">Segar Alami · Sistem Informasi UMKM</td>
