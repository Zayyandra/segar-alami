<x-layouts.admin title="Dashboard" subtitle="Dashboard Sistem Penjualan dan Persediaan UMKM Segar Alami">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm px-6 py-5 mb-4">
        ...
        <p class="text-xs text-slate-500 mb-5">Pendapatan per hari, {{ now()->translatedFormat('F Y') }}</p>
        @if ($trenValues->isEmpty())
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <div class="flex items-center gap-2">
                                <span class="w-2.5 h-2.5 rounded-full"
                                      style="background-color: {{ ['#10b981','#34d399','#6ee7b7','#a7f3d0','#d1fae5'][$i % 5] }}">
                                </span>
    ...
    @if ($volumePerKategori->isNotEmpty() || $distribusi->isNotEmpty() || $trenValues->isNotEmpty())
        <script src="https://cdn.jsdelivr.net/npm/chart.js@4"></script>
        <script>
            const colors = ['#10b981','#34d399','#6ee7b7','#a7f3d0','#d1fae5'];

            @if ($trenValues->isNotEmpty())
            ...
            @if ($volumePerKategori->isNotEmpty())
                new Chart(document.getElementById('chartVolume'), {
                    ...
                    data: {
                        ...
                        datasets: [{
                            data: @json($volumePerKategori->pluck('total')),
            ...
            @if ($distribusi->isNotEmpty())
                new Chart(document.getElementById('chartDistribusi'), {
                    ...
                    data: {
                        ...
                        datasets: [{
                            data: @json($distribusi->pluck('persen')),
                            backgroundColor: colors,
                            borderWidth: 0
                        }]
                    },
